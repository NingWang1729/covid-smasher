...
class Item {
  constructor(store_type) {
    // store_type = unary, binary, ternary
    switch (
      store_type // Will prob change up....
    ) {
      case 'unary':
        this._cost = 1
        break
...
export class Hard_To_Swallow_Pills extends Item {
  ...
  useEffect(player) {
    ...
    swal(
      'Whoops!',
      'You could not swallow these pills! You lost 50% of your current hp.',
      'error'
    )
  ...
  useEffect(player) {
    ...
    swal('Timezones!', 'Eggert was here. GNU is open source.', 'info').then(
      () => {
        swal(
          <p>
            GNU is an operating system that is{' '}
