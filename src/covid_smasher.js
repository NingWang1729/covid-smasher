...
// Helper to calculate distance between player and NPC
function calculateDistance(player, npc) {
  return Math.hypot(player.get_x_pos()-npc.get_x_pos(), player.get_y_pos()-npc.get_y_pos())
}

function isTooClose(player, npc, distance, ticks) {
    // console.log(player.get_x_pos,npc.get_x_pos, player.get_y_pos,npc.get_y_pos)
    if (calculateDistance(player, npc) <= distance) {
        // alert("You are not social distancing!");
        if (ticks % 2 === 0) player.delta_health = -1;
    }
}
...
function COVID_SMASHER() {
    ...
    const [MAX_WIDTH, MAX_HEIGHT] = [1280, 768];
